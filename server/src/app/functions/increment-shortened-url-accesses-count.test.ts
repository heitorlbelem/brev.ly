...
describe('increment shortened url accesses count', () => {
  it('should be able to increment a shortened url accesses count', async () => {
    ...
    const sut = await incrementShortenedUrlAccessesCount({ shortenedUrl: shortenedUrl1.shortenedUrl })
    expect(isRight(sut)).toBe(true)

    const result = await db.select({accessesCount: schema.shortenedUrls.accessesCount})
  ...
  it('should not be able to increment accesses count for a shortened url that does not exist', async () => {
    const shortenedUrl1 = await makeShortenedUrl()
    const sut = await incrementShortenedUrlAccessesCount({ shortenedUrl: shortenedUrl1.shortenedUrl.concat('invalid') })
